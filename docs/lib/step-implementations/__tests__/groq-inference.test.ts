import { beforeEach, describe, expect, it, vi } from 'vitest';
import { groqInference } from '../groq-inference';
import { mockFetchSuccess, mockFetchError } from './setup';

vi.mock('workflow', () => ({
  FatalError: class FatalError extends Error {
    constructor(message: string) {
      super(message);
      this.name = 'FatalError';
    }
  },
}));

const { FatalError } = await import('workflow');

describe('groqInference', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    process.env.GROQ_API_KEY = 'test-api-key';
  });

  it('should throw FatalError if GROQ_API_KEY is missing', async () => {
    delete process.env.GROQ_API_KEY;

    await expect(
      groqInference({ messages: [{ role: 'user', content: 'Hello' }] })
    ).rejects.toThrow(FatalError);
  });

  it('should successfully send a message', async () => {
    mockFetchSuccess({
      choices: [{ message: { content: 'Response text' } }],
    });

    const result = await groqInference({
      messages: [{ role: 'user', content: 'Hello' }],
    });

    expect(fetch).toHaveBeenCalledWith(
      'https://api.groq.com/openai/v1/chat/completions',
      expect.objectContaining({
        method: 'POST',
        headers: {
          Authorization: 'Bearer test-api-key',
          'Content-Type': 'application/json',
        },
      })
    );

    expect(result).toBe('Response text');
  });

  it('should use custom model when provided', async () => {
    mockFetchSuccess({
      choices: [{ message: { content: 'Response' } }],
    });

    await groqInference({
      messages: [{ role: 'user', content: 'Test' }],
      model: 'mixtral-8x7b-32768',
    });

    const callArgs = (fetch as any).mock.calls[0][1];
    const body = JSON.parse(callArgs.body);
    expect(body.model).toBe('mixtral-8x7b-32768');
  });

  it('should throw FatalError on API error', async () => {
    mockFetchError(429, 'Rate limit exceeded');

    await expect(
      groqInference({ messages: [{ role: 'user', content: 'Test' }] })
    ).rejects.toThrow('Groq API error: 429');
  });
});
